...
{
    s->v = 1;
	/*	changed the mutex default vars */
    if (pthread_mutex_init (&(s->mutex), PTHREAD_MUTEX_DEFAULT) == ERROR_CODE)
     do_error ("Error setting up semaphore mutex");
	 
	/*	changed to zero */
    if (pthread_cond_init (&(s->cond), 0) == ERROR_CODE)
...
{
    if (pthread_mutex_destroy (&(s->mutex)) == ERROR_CODE)
     do_error ("Error destroying semaphore mutex");

    if (pthread_cond_destroy (&(s->cond)) == ERROR_CODE)
...
tw_pthread_mutex_unlock (pthread_mutex_t * m)
{
    ...
    if ((return_value = pthread_mutex_unlock (m)) == ERROR_CODE)
...
tw_pthread_mutex_lock (pthread_mutex_t * m)
{
    ...
    if ((return_value = pthread_mutex_lock (m)) == ERROR_CODE)
...
tw_pthread_cond_wait (pthread_cond_t * c, pthread_mutex_t * m)
{
    ...
    if ((return_value = pthread_cond_wait (c, m)) == ERROR_CODE)
...
tw_pthread_cond_signal (pthread_cond_t * c)
{
    ...
    if ((return_value = pthread_cond_signal (c)) == ERROR_CODE)
